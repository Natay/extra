...
def blog_edit(request, id):
    ...
    blog = Blog.objects.filter(id=id).first()
    initial = dict(content=blog.content, privacy=blog.privacy, title=blog.title)
    form = Blogform(initial=initial, user=user)

    if request.method == "POST":
        form = Blogform(data=request.POST, user=user)
        if form.is_valid():
            form.edit(blog=blog)
            return redirect(reverse('blog_view', kwargs=dict(id=id)))

    context = dict(blog=blog, form=form)

    return render(request, 'blog_edit.html', context=context)
